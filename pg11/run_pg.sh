...
if [ ! -d /var/lib/postgresql/11/main ]
then
  /usr/lib/postgresql/11/bin/initdb /var/lib/postgresql/11/main
  perl -p -i -e 's/ssl = on/ssl = off/' /var/lib/postgresql/11/main/postgresql.conf
  echo "host all  all    0.0.0.0/0  md5" > /var/lib/postgresql/11/main/pg_hba.conf
  echo "local all all ident" >> /var/lib/postgresql/11/main/pg_hba.conf
